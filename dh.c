...
int initFromScratch(size_t qbits, size_t pbits)
{
	...
	do {
		do {
    if (fread(qCand, 1, qLen, f) != qLen) {
        fprintf(stderr, "Error reading qCand\n");
        fclose(f);
        free(qCand);
        free(rCand);
        return -1;
...
/* now try to get p */
if (fread(rCand, 1, rLen, f) != rLen) {
    fprintf(stderr, "Error reading rCand\n");
    fclose(f);
    free(qCand);
    free(rCand);
    return -1;
...
do {
    if (fread(tCand, 1, tLen, f) != tLen) {
        fprintf(stderr, "Error reading tCand\n");
        fclose(f);
        free(qCand);
        free(rCand);
        free(tCand);
...
} while (mpz_cmp_ui(g, 1) == 0); /* since q prime, any such g != 1 will be a generator */
...
int dhGen(mpz_t sk, mpz_t pk)
{
    ...
    if (fread(buf, 1, buflen, f) != buflen) {
        fprintf(stderr, "Error reading random buffer\n");
        fclose(f);
        free(buf);
        return -1;
    }
